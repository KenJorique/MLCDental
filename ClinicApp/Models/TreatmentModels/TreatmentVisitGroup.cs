...
public class TreatmentRowItem
{
    ...
    // This treatment's own note — blank if it's just an auto-generated template, so only a
    // genuine staff-typed note (e.g. from the odontogram's edit modal) ever shows.
    public string Notes => IsAutoGeneratedNote(_record.Notes) ? string.Empty : _record.Notes!;

    // Whether this specific row has a genuine note to show.
